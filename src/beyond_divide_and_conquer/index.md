...
分割統治の根っこにあるのが**還元主義**。「複雑な現象は、それを構成するより単純な要素に分解すれば理解・説明できる」という立場 — 分割統治は、その工学における実装
...
| | セクション | 内容 |
|---|---|---|
| §1 | 出来事 | 「なんともならない」— 5つの還元が同時発生する症状 |
| §2 | パターン | 繰り返す失敗の形 — 3つのシステム原型 |
...
分解した瞬間に、**部品の間にあった相互作用が視界から消える**こと

*相互作用が支配的な問題では*、これが致命傷になる
...
§1で見た限定合理性とは、**各部門が盤面を見ずに自分の駒だけを動かしている状態**のことだった
...
私たちが戦略資料と呼ぶものの多くは、**「動き」か「アンカー」が欠けた図**であって、地図ではない。KPIツリーもロードマップも
...
「貢献が見えない」という水面の出来事から潜り、パターン → 構造 → メンタルモデルまで降りて、海底の源流に触れた

介入は深いほど効く。そして一番深い場所にあったのは、**自分のバイアス**だった
...
ただし、相互作用が支配的な対象（組織、事業、ソシオテクニカルシステム）では、**相互作用を捨象した時点で本質を見失う** — 主張は「脱却」ではなく**「適用範囲の認識」**
